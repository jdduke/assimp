...
LPWSTR getCmdOption(LPWSTR* begin, LPWSTR* end, const std::wstring& option)
{
	LPWSTR* itr = std::find(begin, end, option);
	if (itr != end && ++itr != end)
	{
		return *itr;
	}
					 ...
					 int nShowCmd )                // Window Show State
{
	...
	if (argv != NULL && argc > 1)
	{
		std::wstring cmdLinePathString(argv[1]);
		modelpath = std::string(cmdLinePathString.begin(), cmdLinePathString.end());
	}
